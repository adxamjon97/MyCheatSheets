...
if not exists MOni tekshirib ko'radi buday jadval mavjudmi deb va
hechqanday o'zgarish bo'lmaganmi deb tekshiradi.
...
# Если его нужно загрузить в базу данных, 
# тогда подойдет следующий формат:

cur.execute("INSERT INTO users VALUES(?, ?, ?, ?);", user)
conn.commit()
```

bir qancha ma'lumot saqlash
```py
...
barcha ma'lumotlarni oladi userdan.
